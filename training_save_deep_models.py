...
from models.ResAFNet import AFNet
import time
...
def main():
    ...
    for epoch in range(epoch_num):  # loop over the dataset multiple times (specify the #epoch)
        ...
        torch.save(net, './saved_models/ECG_net_'+str(epoch)+'.pkl')
        torch.save(net.state_dict(), './saved_models/ECG_net_state_dict_'+str(epoch)+'.pkl')

    file = open('./saved_models/loss_acc.txt', 'a')
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--path_data', type=str, default='./data_ECGdb_ICMCdb/training_dataset/')
